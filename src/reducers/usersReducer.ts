...
export const getUsersTC = createAsyncThunk('users/getUsers' , async (param , {dispatch}) => {
...
export const modifyUserTC = createAsyncThunk('users/modifyUser' , async (param: UserType , {dispatch}) => {
    dispatch(setAppStatusAC({status: 'loading'}))
    try {
        await fetch(`${endpoint}/users/:${param.id}` , {
            method: 'PATCH' ,
...
export const addNewUserTC = createAsyncThunk('users/addUser' , async (param: UserType , {dispatch}) => {
    dispatch(setAppStatusAC({status: 'loading'}))
    try {
